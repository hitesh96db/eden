...
(function () {
    ...
    S3.imageCropWidget = function(type) {
        ...
        var FileHoverHandler = function(e) {
            e.stopPropagation();
            e.preventDefault();
            e.target.className = 'imagecrop-drag-' + type;
            if (e.type == 'dragover') {
                e.target.className = 'imagecrop-drag-' + type + '-hover';
            }
        };
        ...
        var loadImage = function(data) {
            ...
            image.onload = function() {
                ...
                if (toScale) {
                    $('.imagecrop-data-' + type).val(fileName + ';' + scaled_image);
                }
                else {
                    // Don't Scale
                    $('.imagecrop-data-' + type).val(fileName + ';' + data);
                }
                $('#uploaded-image-' + type).attr({
                    src: scaled_image,
                    style: 'display: block;',
                });
                $('#select-crop-btn-' + type).css({
                    display: 'inline'
                });
                $('hr').attr({
                    style: 'display:block'
        ...
        var $uploadTitle = $('#upload-title-' + type),
            $uploadContainer = $('#upload-container-' + type);
        ...
        var FileSelectHandler = function(e) {
            ...
            var $upload = $('.imagecrop-upload-' + type);
        ...
        var $selectCrop = $('#select-crop-btn-' + type),
            $crop = $('#crop-btn-' + type),
            $cancel = $('#remove-btn-' + type),
        ...
        $('#uploaded-image-' + type).bind('load', EnableCrop);
  
        // Image already stored in DB ( Update form )
        // load the Image
        
        var imgData = $('.imagecrop-data-' + type).attr('value');
    ...
    //            loadImage(data);
        ...
        $('.imagecrop-drag-' + type).bind('dragover', FileHoverHandler)
                                    .bind('dragleave', FileHoverHandler)
                                    .bind('drop', FileSelectHandler);
        $('.imagecrop-upload-' + type).bind('change', FileSelectHandler); 
    };
}());
